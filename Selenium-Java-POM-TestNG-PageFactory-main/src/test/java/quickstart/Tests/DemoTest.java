...
import quickstart.Pages.HomePage;
import quickstart.Pages.SearchResultPage;
...
//		driver.quit();
	...
	public  void Booking() throws IOException {
		...
		String from=testdata.getTestData("1", "Keyword");
		String To=testdata.getTestData("1", "Keyword");
		homePage.fillJourneyDetails(from,To);			
		boolean isDisplayed= searchresults.istextDisplayed("PNR Enquiry");		
		if(isDisplayed==true) {
			logger.logPass("Text is displayed on Search results page 1");
		}else {
			logger.logFail("Text  is not displayed on Search results page 1");
